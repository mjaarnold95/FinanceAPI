...
class PayPeriodFrequency(StrEnum):
    WEEKLY = "weekly"
    BIWEEKLY = "biweekly"
...
class PayStub(SQLModel, table=True):
    ...
    notes: str | None = None
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    pay_period: PayPeriod | None = Relationship(back_populates="pay_stubs")
...
class PayStubLineItem(SQLModel, table=True):
    ...
    pay_stub: PayStub | None = Relationship(back_populates="line_items")
